...
//         db.query('SELECT `user_id`, `name`, `email`, `password`, `mobile`, `address`, `image`, `device_type`, `auth_token`, `status`  FROM `user_detail` WHERE `auth_token` = ? AND `status` = ?', [headerObj.access_token, "1"], (err, result) => {
//             if (err) {
//                 helper.ThrowHtmlError(err, res);
//                 return;
//             }
            ...
            form.parse(req, (err, reqObj, files) => {
                ...
                helper.CheckParameterValid(res, files, ["image"], () => {
                    ...
                    fs.copyFile(files.image[0].path, newPath, (err) => {
                        ...
                        fs.unlink(files.image[0].path, (unlinkErr) => {
                            if (unlinkErr) {
                                helper.ThrowHtmlError(unlinkErr, res);
                                return;
                            }
